...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("global_status IN ('ACTIVE', 'INTERVIEWING', 'EXPIRED', 'INELIGIBLE', 'DELETED')", name='ck_candidates_global_status'),
    )
    ...
    op.create_table(
        ...
        sa.Column('updated_by', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('deleted_by', postgresql.UUID(as_uuid=True), nullable=True),
        sa.CheckConstraint('parse_status IN (\'PENDING\', \'COMPLETED\', \'FAILED\')', name='ck_resumes_parse_status'),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("source IN ('MANUAL', 'PARSED', 'INFERRED')", name='ck_candidate_skills_source'),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("designation IN ('REQUIRED', 'DESIRED')", name='ck_job_profile_skills_designation'),
    ...
    op.create_table(
        ...
        sa.Column('updated_by', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('deleted_by', postgresql.UUID(as_uuid=True), nullable=True),
        sa.CheckConstraint("status IN ('OPEN', 'ON_HOLD', 'CLOSED', 'CANCELLED')", name='ck_job_requisitions_status'),
    ...
    op.create_table(
        ...
        sa.Column('updated_by', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('deleted_by', postgresql.UUID(as_uuid=True), nullable=True),
        sa.CheckConstraint("request_type IN ('ACCESS', 'ERASURE')", name='ck_data_subject_access_requests_request_type'),
        sa.CheckConstraint("status IN ('PENDING', 'PROCESSING', 'COMPLETED', 'DENIED')", name='ck_data_subject_access_requests_status'),
