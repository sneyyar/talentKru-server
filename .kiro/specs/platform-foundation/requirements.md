...
3. THE Server SHALL expose a health check endpoint at GET /health that returns a JSON response containing a status field (value "healthy" when the application and database connection are operational, "unhealthy" otherwise, or when any additional platform-defined conditions indicate degraded state) and a version field (the application semantic version string).
...
1. THE Server SHALL store Organization entities with fields: OrganizationID (UUID), Name (maximum 128 characters), slug (lowercase alphanumeric and hyphens only, maximum 64 characters, unique across all organizations), branding settings (logo URL, primary color, secondary color, terms and conditions URL), primary contact details (contact_name maximum 128 characters, contact_email a valid email address format, contact_phone maximum 32 characters), feature flags (stored as a JSON object of string keys to boolean values), shard_id, and AuditFields.
...
6. THE Server SHALL support creating, updating, listing, and retrieving organizations via REST endpoints restricted to the SuperAdministrator role; SuperAdministrators SHALL be permitted to manage organizations across different tenants, bypassing the cross-org restriction that applies to all other roles.
...
1. THE Server SHALL publish domain events to an internal event bus implemented using FastAPI BackgroundTasks for immediate processing and a persistent event log table (DomainEvent) for audit and retry purposes; WHEN FastAPI BackgroundTasks is unavailable (e.g., during system stress or maintenance), THE Server SHALL still persist the event to the DomainEvent table with Status set to Pending for later processing.
...
5. IF a domain event handler fails, THEN THE Server SHALL update the DomainEvent Status to Failed and log the error with the EventID and correlation ID; the Failed status update SHALL be persisted even if the error logging step itself fails, and the event SHALL be retained for manual retry or automated retry via a scheduled background task.
...
3. THE Server SHALL restrict access to the metrics endpoint (GET /metrics) using HTTP Basic Authentication with credentials validated against METRICS_USERNAME and METRICS_PASSWORD environment variables; IF authentication fails, THE Server SHALL explicitly deny access with a 401 Unauthorized response.
...
4. IF a request targets an internal agent endpoint (any route under the /internal/agents/ path prefix) without a valid X-Agent-API-Key header, THEN THE Server SHALL return a 401 Unauthorized response; IF the AGENT_API_KEY environment variable is not defined or is empty, THE Server SHALL reject all requests to internal agent endpoints with a 401 Unauthorized response.
...
3. IF a CORS request originates from an origin not in the organization's allowed origins list, THEN THE Server SHALL omit CORS headers from the response, causing the browser to block the request, and SHALL log the unauthorized CORS attempt at WARN level with the requesting origin and the target organization identifier.
...
4. IF the submitted Version does not match the stored Version, THEN THE Server SHALL reject the update with a 409 Conflict response containing an error message indicating the resource has been modified by another request, the current Version value, and a recommendation to re-fetch and retry; THE Server SHALL guarantee mutual exclusivity — either the update is accepted and persisted OR the 409 Conflict response is returned, but never both for the same request.
